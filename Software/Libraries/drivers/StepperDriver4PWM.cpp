...
int StepperDriver4PWM::init() {
  ...
  //_configure4PWM(pwm_frequency, pwm1A, pwm1B, pwm2A, pwm2B);
	PWM_Init(pwm1A, 1000, pwm_frequency);
	PWM_Init(pwm1B, 1000, pwm_frequency);
	PWM_Init(pwm2A, 1000, pwm_frequency);
	PWM_Init(pwm2B, 1000, pwm_frequency);
  return 0;
}
...
void StepperDriver4PWM::setPwm(float Ualpha, float Ubeta) {
  ...
  pwmWrite(pwm1A, duty_cycle1A*_PWM_RANGE);
	pwmWrite(pwm1B, duty_cycle1B*_PWM_RANGE);
	pwmWrite(pwm2A, duty_cycle2A*_PWM_RANGE);
	pwmWrite(pwm2B, duty_cycle2B*_PWM_RANGE);
